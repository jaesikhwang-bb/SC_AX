...
class PromptBundleLoader:
    ...
    def load(self, requested_version: str | None = None) -> PromptBundle:
        ...
        refinement_path = version_dir / manifest["router"]["refinement_prompt"]
        if not refinement_path.is_file():
            raise FileNotFoundError(f"질문 보정 프롬프트 파일이 없습니다: {refinement_path}")
        ...
        # 보정 프롬프트는 별도 LLM 체인에서만 사용한다. 에이전트 분류 시스템
        # 프롬프트에 다시 합치면 두 단계의 책임이 섞이므로 자동 결합에서도 제외한다.
        declared = {path.resolve() for path in paths}
        declared.add(refinement_path.resolve())
        paths.extend(
            path
        ...
        bundle = PromptBundle(
            version=version,
            refinement_prompt=refinement_path.read_text(encoding="utf-8").strip(),
            system_prompt="\n\n---\n\n".join(parts),
